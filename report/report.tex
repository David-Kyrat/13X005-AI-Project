...
\begin{align*}
\frac{\partial \log(y*i|\mathbf{x}*i ; \mathbf{w}, b)}{\partial b}
&= y_i - \sigma(z_i)
= y_i - \sigma(\mathbf{w}^T X_i + b)\\
%
\frac{\partial \log(y_i|\mathbf{x}_i ; \mathbf{w}, b)}{\partial w_j}
&= x*{ij}* ( y*i - \sigma(z*i))
= (y_i - \sigma(\mathbf{w}^T X_i + b)) \* x*{ij}
\end{align\*}
...
\code{log_reg.py} de signature suivante:
...
\begin{quote}
N.B.:\\
L'apprentissage peut être ré-effectué de manière efficace si besoin à
l'aide du jupyter notebook
...
\caption{Comparaison des distributions réelles et échantillonées pour la
classe 0}
\end{figure}
...
Notre Naive Bayes - Precision: 0.9761904761904763 - Recall:
0.9743589743589745 - Accuracy: 0.9777777777777777 - F1\_score:
0.9752738654147106
...
On voit que les résultats sont identiques, ce qui veut dire que notre
...
Notre Logistic Regression - Precision: 0.8505050505050505 - Recall:
0.8461538461538461 - Accuracy: 0.8666666666666667 - F1\_score:
0.848323868840447

Sklearn Logistic Regression - Precision: 0.9761904761904763 - Recall:
0.9743589743589745 - Accuracy: 0.9777777777777777 - F1\_score:
0.9752738654147106
